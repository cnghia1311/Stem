...
function TokenChecklist({
  ...
  const addCustom = () => {
    ...
    if (!/^0x[a-fA-F0-9]{40}$/.test(a)) {
      alert("Địa chỉ contract không hợp lệ (phải là 0x + 40 ký tự hex)");
      return;
    }
    if (value.some((x) => x.toLowerCase() === a.toLowerCase())) {
      alert("Coin này đã có trong danh sách rồi");
      return;
    }
    onChange([...value, a]);
    setCustom("");
  ...
  const presetValues = options.map((o) => o.value.toLowerCase());
  const customTokens = value.filter(
    (v) => !presetValues.includes(v.toLowerCase()),
  );

  return (
    <div className="space-y-2">
      {/* Danh sách coin dựng sẵn */}
      <div className="space-y-0.5 p-2 rounded-lg bg-secondary/40 border border-border">
        {options.map((opt) => (
          <label
            key={opt.value}
      ...
      {customTokens.length > 0 && (
        <div className="space-y-0.5 p-2 rounded-lg bg-secondary/40 border border-border">
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground px-2 pb-1">
            Coin tự thêm
          </p>
          {customTokens.map((addr) => (
            ...
            >
              <span className="text-xs font-mono text-foreground truncate">
                🪙 {addr.substring(0, 10)}...{addr.slice(-6)}
              </span>
              <button
                type="button"
                onClick={() => toggle(addr)}
      ...
      {/* Ô dán địa chỉ coin tự deploy */}
      <div className="flex gap-2">
        <Input
          placeholder="Dán địa chỉ coin (0x...)"
          value={custom}
        ...
        <button
          type="button"
          onClick={addCustom}
          className="shrink-0 px-3 rounded-lg bg-[#10b981] text-white text-xs font-bold hover:opacity-90"
        >
          Thêm
        </button>
      </div>
      ...
      <p className="text-[11px] text-muted-foreground">
        Đã chọn <b className="text-foreground">{value.length}</b> coin — sẽ hiện
        sẵn trong dropdown của dApp khi export.
      </p>
    </div>
  );
}
interface InspectorPanelProps {
  ...
  onUpdateContract: (blockId: string, fieldKey: string, value: any) => void;
  onUpdateDecorative: (
    instanceId: string,
    fieldKey: string,
    value: any,
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {selectedBlock
                ? "Cài đặt thông số cho Smart Contract"
            ...
            <div className="p-4 rounded-lg bg-secondary/30 border border-border flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Khối sẽ trôi nổi trên màn hình và không bị cuộn.
                </p>
              </div>
              <Switch
                id="fixed-toggle"
            ...
            <div className="space-y-4 border-t border-border pt-6">
              {selectedBlock.blockType === "logic" ? (
                <>
                  ...
                  {(() => {
                    if (
                      selectedBlock.contractFields &&
                      selectedBlock.contractFields.length > 0
                    ) {
                      return selectedBlock.contractFields.map((field, idx) => (
                        <div
                          className="space-y-2"
                          key={`${selectedBlock.id}-${idx}`}
                        >
                          <Label
                          ...
                          {field.type === "token-checklist" ? (
                            <TokenChecklist
                              options={field.options || []}
                              value={
                                Array.isArray(
                                  contractsConfig[selectedBlock.id]?.[
                                    field.key
                                  ],
                                )
                                  ? contractsConfig[selectedBlock.id][field.key]
                                  : []
                              }
                              onChange={(next) =>
                                onUpdateContract(
                    ...
                    if (
                      selectedBlock.uiFields &&
                      selectedBlock.uiFields.length > 0
                    ) {
                      return (
                        <div className="mt-6">
                          <h4 className="text-sm font-bold text-foreground mb-4 flex items-center gap-2">
                          ...
                          {selectedBlock.uiFields.map((field, idx) => (
                            <div
                              ...
                              key={`ui-${selectedBlock.id}-${idx}`}
                            >
                              <Label
                                htmlFor={`ui-${field.key}`}
                                className="text-sm font-medium text-foreground"
                              >
                                {field.label}
                              </Label>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      onUpdateAppConfig?.("showTabBar", checked)
                    }
                    className="data-[state=checked]:bg-neon-blue"
                  />
                </div>
                <p className="text-xs text-muted-foreground">
                  Khi tắt, thanh chuyển hướng ở dưới đáy DApp sẽ bị ẩn. Hãy đảm
                  bảo bạn đã tạo các "Nút Chuyển Trang" thay thế!
                </p>
              </div>
            </div>
          </div>
